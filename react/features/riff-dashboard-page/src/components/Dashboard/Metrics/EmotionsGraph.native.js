...
//amcharts not compatible with react-native
...
function EmotionsGraph({ data = [], participantId, startTime, endTime, dashboardGraphLoaded }) {
  const [bandsData, setBandsData] = useState([]);
  const [currentUserData, setCurrentUserData] = useState([])
  const [spikes, setSpikes] = useState([]);
  const [domainYxis, setDomainYAxis] = useState({ min: -1, max: 1 });
