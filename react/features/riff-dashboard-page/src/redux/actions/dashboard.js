...
function loadMeetingData(uid, meetingId) {
    async function thunk(dispatch /*, getState*/) {
        ...
        dispatch({ type: ActionTypes.DASHBOARD_FETCH_MEETING_STATS, status: 'loading' });
        dispatch({ type: ActionTypes.DASHBOARD_FETCH_MEETING_TIMELINE, status: 'loading' });
        dispatch({ type: ActionTypes.DASHBOARD_FETCH_MEETING_INFLUENCE, status: 'loading' });
        dispatch({ type: ActionTypes.DASHBOARD_FETCH_MEETING_INTERRUPTIONS, status: 'loading' });
        dispatch({ type: ActionTypes.DASHBOARD_FETCH_MEETING_AFFIRMATIONS, status: 'loading' });
...
function roomIdToName(idsToNames) {
    return (mtg) => {
        if (Object.prototype.hasOwnProperty.call(idsToNames, mtg.room)) {
            return { ...mtg, room: idsToNames[mtg.room] };
        }
