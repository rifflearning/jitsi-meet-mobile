...
import { Colors, formatDuration, getColorForSelf } from '../../libraries/utils';
...
const GraphConfigs = {
    [GraphTypes.MY_INFLUENCE]: {
        ...
        eventsFilter: (events, uid) => events.filter(event => event.earlierUtt.utt.participant === uid).map(event => event.earlierUtt),
        getSourceParticipant(/* event */) {
            return 'You';
        },
        getTargetParticipant(event) {
            return event.otherParticipantName;
        },
        getEventTime(event) {
            return event.utt.endTime;
        },
        getTooltip() {
    ...
    [GraphTypes.THEIR_INFLUENCE]: {
        ...
        eventsFilter: (events, uid) => events.filter(event => event.laterUtt.utt.participant === uid).map(event => event.laterUtt),
        getSourceParticipant(event) {
            return event.otherParticipantName;
        },
        getTargetParticipant(/* event */) {
    ...
    [GraphTypes.THEIR_INTERRUPTIONS]: {
        ...
        getEventTime(event) {
            return event.otherUtt.startTime;
        },
        getTooltip() {
