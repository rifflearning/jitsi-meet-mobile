...
const participantLeaveRoom = (meetingId, participantId) => app.service('meetings').patch(meetingId, {
...
const riffCreateParticipant = async user => app.service('participants').create({
    _id: user.uid,
    consent: true,
...
const riffCreatePersonalRoom = async user => app.service('personalRooms').create({
    owner: user.uid,
    title: user.roomName
});

const riffChangeRoomName = async (uid, roomName) => app.service('personalRooms').patch(
        null,
        { title: roomName },
        { query: { owner: uid } }
);
