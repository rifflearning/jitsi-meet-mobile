...
function addWebRtcListeners(nick, localVideoId, dispatch, getState) {
    ...
    const webRtcConfig = {
        ...
        debug: Boolean(window.client_config.webrtc_debug)
    };
    ...
    webrtc.on('screenRemoved', (video, peer) => {
        logger.debug('Listener.WebRtc.screenRemoved: removing shared screen!', { video,
            peer });
    ...
    webrtc.on('localStreamRequestFailed', () => {
    ...
    webrtc.on('readyToCall', connectionId => {
        dispatch(getMediaError(false));
